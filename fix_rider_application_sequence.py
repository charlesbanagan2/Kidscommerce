"""
Fix RiderApplication sequence and create missing records
"""
import os
import sys
...
with app.app_context():
    print("=" * 80)
    print("FIXING RIDER APPLICATION SEQUENCE")
    print("=" * 80)
    
    # Fix the sequence
    try:
        # Get the max ID from rider_application table
        max_id = db.session.execute(text("SELECT MAX(id) FROM rider_application")).scalar()
        if max_id:
            # Set the sequence to max_id + 1
            db.session.execute(text(f"SELECT setval('rider_application_id_seq', {max_id}, true)"))
            db.session.commit()
            print(f"\n✓ Sequence fixed. Next ID will be: {max_id + 1}")
        else:
            print("\n✓ No records in rider_application table yet.")
