# Province Delivery Fee Calculator
# Base location: Laguna
# Formula: Province Rank × 36 pesos
...
CITY_TO_PROVINCE = {
    ...
    'Alfonso': 'Cavite', 'Indang': 'Cavite', 'Trece Martires': 'Cavite',
    'General Trias': 'Cavite', 'Dasmarinas': 'Cavite',
}
...
def calculate_delivery_fee(province_or_address):
    """Calculate delivery fee based on province name or address"""
    # If it's an address (contains spaces or commas), extract province
    if ' ' in province_or_address or ',' in province_or_address:
        province = extract_province_from_address(province_or_address)
        if not province:
            return 36.0  # Default to Laguna
    else:
        province = province_or_address
    
    rank = PROVINCE_RANKS.get(province, 1)
    return float(rank * BASE_FEE)

def calculate_delivery_fee_from_address(address):
    """Calculate delivery fee directly from address"""
...
def get_province_rank(province):
    """Get province rank"""
    return PROVINCE_RANKS.get(province, 1)
