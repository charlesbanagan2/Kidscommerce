...
from app import app, db, socketio, User, Order, OrderItem, SellerApplication, RiderApplication, token_required, role_required, push_notification
...
def api_rider_register():
    ...
    try:
        ...
        rider_application = RiderApplication(
            user_id=user.id,
        ...
        db.session.add(rider_application)
        db.session.commit()
...
def api_rider_available_orders():
    ...
    try:
        # Check if rider is active/approved
    ...
    try:
        ...
        try:
            socketio.emit('order_accepted_by_rider', {
                ...
                'rider_profile_picture': getattr(rider, 'profile_picture', None),
                'status': 'in_transit'
...
# ============================================
# RIDER DECLINE ORDER API
# ============================================

@app.route('/api/v1/rider/decline-order', methods=['POST'])
@token_required
@role_required('rider')
def api_rider_decline_order():
    ...
    try:
        ...
        # Notify all riders
        try:
            socketio.emit('order_declined', {
                'order_id': order.id
            }, room='riders')
        except Exception as e:
            app.logger.error(f"Error broadcasting order_declined: {str(e)}")
...
def api_rider_upload_proof(order_id):
    ...
    try:
        ...
        # Save file
        filename = secure_filename(f"proof_{order_id}_{datetime.utcnow().timestamp()}_{file.filename}")
        filepath = os.path.join(app.config['UPLOAD_FOLDER'], filename)
        file.save(filepath)
        
        # Update order
        order.proof_photo_url = filename
        order.updated_at = datetime.utcnow()
        db.session.commit()
        ...
        db.session.rollback()
        app.logger.error(f"Error uploading proof photo: {str(e)}")
        return jsonify({
            'success': False,
...
@app.route('/api/v1/rider/notifications', methods=['GET'])
@token_required
@role_required('rider')
def api_rider_get_notifications():
...
@app.route('/api/v1/rider/notifications/<int:notification_id>/read', methods=['POST'])
@token_required
@role_required('rider')
def api_rider_mark_notification_read(notification_id):
...
@app.route('/api/v1/rider/notifications/mark-all-read', methods=['POST'])
@token_required
@role_required('rider')
def api_rider_mark_all_notifications_read():
...
@app.route('/api/v1/rider/notifications/unread-count', methods=['GET'])
@token_required
@role_required('rider')
def api_rider_unread_notifications_count():
