-- =============================================
-- FIX MISSING AND LIMITED RLS POLICIES
-- =============================================
...
-- Users can view their follows and followers
CREATE POLICY "Users can view store followers" ON "store_follower"
  FOR SELECT USING (
    follower_id = current_setting('app.user_id', true)::bigint
    OR store_id = current_setting('app.user_id', true)::bigint
  );
...
-- ---------------------------------------------
-- 7. STORE_RATING TABLE
-- ---------------------------------------------

-- Anyone can view published store ratings
CREATE POLICY "Anyone can view store ratings" ON "store_rating"
  FOR SELECT USING (
    status = 'published'
  );

-- Users can view their own ratings (including pending)
CREATE POLICY "Users can view own store ratings" ON "store_rating"
  FOR SELECT USING (
    user_id = current_setting('app.user_id', true)::bigint
  );
...
GRANT SELECT, INSERT, DELETE ON store_follower TO authenticated;

GRANT SELECT, INSERT, UPDATE, DELETE ON store_rating TO authenticated;
